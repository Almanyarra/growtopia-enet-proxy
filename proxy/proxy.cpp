...
void clearHost() {
    std::ofstream ofs("C:\\Windows\\System32\\drivers\\etc\\hosts", std::ios::trunc);
    if (ofs.is_open()) {
        printf("Host file fixed\n");
    }
    else {
        printf("Host File Fix Error\n");
    }
}
...
void setHost() {
    ...
    if (sethost) {
        ...
        std::cout << "Wrote Host File" << std::endl;
    }
    else {
        std::cerr << "Failed to open host file for writing" << std::endl;
    }
}

int main() {
#ifdef _WIN32
    SetConsoleTitleA("Proxy by MegaMini#9735");
    SetConsoleCtrlHandler(exit_handler, true);
#endif
    printf("Proxy by MegaMini#9735\n");
    clearHost();
    Sleep(1000);
    setHost();
    ...
    enet_initialize();
    if (g_server->start()) {
        printf("Server & client is running.\n");
        while (true) {
            g_server->poll();
